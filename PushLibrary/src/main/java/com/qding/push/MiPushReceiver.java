...
//        UpdateHandler.updateContent(2,"{"+message.getTitle()+";"+mMessage+";"+message.getExtra()+"}");
    ...
    public void onNotificationMessageArrived(Context context, MiPushMessage message) {
        mMessage = message.getContent();
        String log = "{title:"+message.getTitle()+";content:"+mMessage+";extra:"+message.getExtra()+"}";
        Log.e("DDAI_MI", "on Notification Arrived = "+log);
        //UpdateHandler.updateContent(2,"Arrived="+message.getExtra());
        if(!TextUtils.isEmpty(message.getTopic())) {
            mTopic=message.getTopic();
        } else if(!TextUtils.isEmpty(message.getAlias())) {
            mAlias=message.getAlias();
    ...
    public void onCommandResult(Context context, MiPushCommandMessage message) {
        Log.e("DDAI_MI", "onCommandResult = "+message);
    ...
    public void onReceiveRegisterResult(Context context, MiPushCommandMessage message) {
        Log.e("DDAI_MI", "onReceiveRegisterResult = "+message);
        String command = message.getCommand();
        List<String> arguments = message.getCommandArguments();
        String cmdArg1 = ((arguments != null && arguments.size() > 0) ? arguments.get(0) : null);
        String cmdArg2 = ((arguments != null && arguments.size() > 1) ? arguments.get(1) : null);
